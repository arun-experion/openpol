<?php  
	...
	$tpl = new template();
	$tpl -> Load(TEMPLATE_PATH . "complaintstatus.tpl");
	...
	//Get status
	$statusquery = Query("SELECT * FROM `[x]status` where complaint=1 ");	
	...
	if($complaint->getworkflows()) {
		$tpl -> Loop("workflows", $complaint->getworkflows());
 	}else{	
