<?php 
    ...
    */
	...
	//search case	
        ...
        $statusCondition = "AND u.status REGEXP '1' AND (os.status REGEXP '^1$' OR os.status REGEXP '^3$') AND 1 and o.status = 1 and o.created_date <= NOW() - INTERVAL 15 MINUTE";        
        if($_SESSION['rid'] == 2){
            $statusCondition 	= "AND u.status REGEXP '1' AND (os.status REGEXP '^1$' OR os.status REGEXP '^3$') AND 1 and o.status = 1 and o.created_date <= NOW() - INTERVAL 15 MINUTE";
            $statusflag 		= ' o.status, ';
        }
        if($_GET['ex_status']==1){
	  	...
	  	$s_query 	= "Select distinct(o.id), o.order_no,o.payment_terms,o.customer_id,customer_id,o.payment_term,o.payment_no,o.payment_date,o.payment_amount,$statusflag u.first_name,u.erp_number, DATE_FORMAT(o.created_date , ".SHORT_DATE_WITHTIME." ) as order_date,o.credit, o.created_by,o.prefered_date,o.is_exported,o.instructions,o.exported_by,pa.rate,pa.quantity,pt.name as category_name,p.code
		...
		if(isset($_GET['product_type']) && $_GET['product_type']!='0'){
			$statusCondition = " AND (os.status REGEXP '^1$' OR os.status REGEXP '^3$') AND 1 and o.status = 1 and o.created_date <= NOW() - INTERVAL 15 MINUTE and p.product_type_id = '".$_GET['product_type']."' ";         
			if($_SESSION['rid'] == 2){
				$statusCondition 	= "AND (os.status REGEXP '^1$' OR os.status REGEXP '^3$') AND 1 and o.status = 1 and o.created_date <= NOW() - INTERVAL 15 MINUTE and p.product_type_id = '".$_GET['product_type']."'";
				$statusflag 		= ' o.status, ';
			}
			if($_GET['ex_status']==1){
	  		...
	  		$s 		= new search($searchFields, $condition, $extracondition);		
	  		$query 	= "Select distinct(o.id), o.order_no,o.payment_terms,o.customer_id,o.payment_term,o.payment_no,o.payment_date,o.payment_amount,$statusflag u.first_name,u.erp_number, DATE_FORMAT(o.created_date , ".SHORT_DATE_WITHTIME." ) as order_date,o.credit, o.created_by,o.prefered_date,o.is_exported,o.instructions,o.exported_by,pa.rate,pa.quantity,pt.name as category_name,p.code
			  			From (Select order_id,MAX(current_status) as status from order_status group by order_id) os INNER JOIN `order` o ON o.id = os.order_id INNER JOIN `user` u ON u.id=o.created_by INNER JOIN user_zones uz ON o.created_by = uz.userID INNER JOIN user_areas ua ON o.created_by = ua.userID 
						INNER JOIN product_order pa ON pa.order_id = o.id INNER JOIN product p ON p.id = pa.product_id INNER JOIN product_type pt ON pt.id = p.product_type_id ". $s->query. "  $statusCondition  ORDER BY o.id desc";
		} else {
			$query = $s_query;
	...
	if(Num($q->out)) {
		...
		while($r=FetchAssoc($q->out)){
			...
			if($r['exported_by'] ==0){
				...
				$r['exported_by'] = 	$order->getuserfirstname($r['exported_by']);
			}
...
?>
<form method="post" action="export_csv.php" id="ReportForm">
...
$(function() {
	...
	$('.orderreport').click(function() {
		...
		$.ajax({
			...
			{
				if(data.success == 'yes'){
					var url = "templates/default/csv/"+data.filename;        
					var downloadWindow = window.open(url, "downloadWindow");
					window.setTimeout(function () {
						$.ajax({
							type : 'POST',
							url  : 'remove_export_csv.php',
